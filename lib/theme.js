...
window.Theme = (function () {
  ...
  // The shared bottom-right row. Anything that wants to live in that corner
  // asks for this instead of pinning itself, which is what stopped the sound
  // toggle, the theme toggle and the next-game chip landing on each other.
  function dock() {
    let el = document.querySelector('.fx-dock');
    if (!el && document.body) {
      el = document.createElement('div');
      el.className = 'fx-dock';
  ...
  return { set, cycle, mountButton, dock, get pref() { return stored(); }, get isDark() { return resolved(stored()) === 'dark'; } };
})();
