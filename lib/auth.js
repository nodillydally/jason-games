...
window.Auth = (function () {
  ...
  // Asks the API what this player is allowed to reach. Sync owns the API base,
  // and is loaded right after this file, so the lookup is deliberately lazy —
  // reading it at definition time would run before that script exists.
  async function loadCaps() {
    if (!api.isSignedIn() || !window.Sync) { caps = { owner: false }; return caps; }
    try {
      const res = await fetch(`${Sync.contentEndpoint()}?op=capabilities`, {
