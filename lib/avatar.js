...
window.Avatar = (function () {
  ...
  /* Floating extras: halos, orbiting motes, wings and pets.
   *
   * These are the ones worth drawing once, because they attach to nothing. A
   * hat has to be anchored to a particular head and re-anchored for every new
   * character; a halo just hovers above whatever the top of the sprite is, and
   * `top` is already derived per frame for every sprite we import. So one
   * drawing serves the whole roster, and every character added later.
   *
   * Placement is in fractions of the sprite box rather than pixels, so a
   * 20-wide oracle and a 31-wide Bane both wear it correctly.
   */
  const EXTRAS = {
    wing:  { dir: 'extras', w: 0.92, at: (fr, W, H) => [0.5, (fr.top + 9) / H], behind: true },
    halo:  { dir: 'extras', w: 0.46, at: (fr, W, H) => [fr.cx / W, (fr.top - 1.5) / H] },
    orbit: { dir: 'extras', w: 0.17, at: () => [0.5, 0.45], spin: true, behind: true },
    pet:   { dir: 'pets',   w: 0.4,  at: () => [-0.02, 0.9] },
  };
  const EXTRA_ORDER = ['wing', 'orbit', 'halo', 'pet'];
  ...
  const SPRITES = {
    ...
    berserker:  { dir: 'berserker', w: 22, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 10 }, { f: 's-run-1', top: 0, cx: 10 }, { f: 's-run-2', top: 0, cx: 10 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 0, cx: 10 }] } } },
    drake:      { dir: 'drake', w: 26, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 12 }, { f: 's-run-1', top: 1, cx: 12 }, { f: 's-run-2', top: 0, cx: 12 }, { f: 's-run-3', top: 0, cx: 12 }, { f: 's-run-4', top: 1, cx: 12 }] } } },
    rogue:      { dir: 'rogue', w: 24, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 10 }, { f: 's-run-1', top: 1, cx: 10 }, { f: 's-run-2', top: 0, cx: 10 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 0, cx: 10 }] } } },
    oracle:     { dir: 'oracle', w: 20, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 9 }, { f: 's-run-1', top: 0, cx: 9 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 0, cx: 9 }] } } },
    ronin:      { dir: 'ronin', w: 22, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 10 }, { f: 's-run-1', top: 1, cx: 10 }, { f: 's-run-2', top: 1, cx: 10 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 1, cx: 10 }] } } },
    bonelord:   { dir: 'bonelord', w: 20, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 2, cx: 9 }, { f: 's-run-1', top: 2, cx: 9 }, { f: 's-run-2', top: 1, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 1, cx: 9 }] } } },
    corsair:    { dir: 'corsair', w: 20, h: 28, facings: { s: { run: [{ f: 's-run-0', top: 0, cx: 9 }, { f: 's-run-1', top: 0, cx: 9 }, { f: 's-run-2', top: 1, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 0, cx: 9 }] } } },
    shinobi:    { dir: 'shinobi', w: 24, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 14 }, { f: 's-run-1', top: 1, cx: 11 }, { f: 's-run-2', top: 0, cx: 11 }, { f: 's-run-3', top: 0, cx: 12 }, { f: 's-run-4', top: 1, cx: 13 }] } } },

    sentinel:   { dir: 'sentinel', w: 20, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 9 }, { f: 's-run-1', top: 0, cx: 9 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 1, cx: 10 }, { f: 's-run-4', top: 1, cx: 10 }] } } },
    warchief:   { dir: 'warchief', w: 22, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 10 }, { f: 's-run-1', top: 0, cx: 10 }, { f: 's-run-2', top: 0, cx: 10 }, { f: 's-run-3', top: 1, cx: 10 }, { f: 's-run-4', top: 1, cx: 10 }] } } },
    ranger:     { dir: 'ranger', w: 24, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 11 }, { f: 's-run-1', top: 0, cx: 11 }, { f: 's-run-2', top: 0, cx: 11 }, { f: 's-run-3', top: 0, cx: 11 }, { f: 's-run-4', top: 0, cx: 11 }] } } },
    kitsune:    { dir: 'kitsune', w: 25, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 12 }, { f: 's-run-1', top: 1, cx: 12 }, { f: 's-run-2', top: 0, cx: 12 }, { f: 's-run-3', top: 0, cx: 12 }, { f: 's-run-4', top: 1, cx: 12 }] } } },
    infernal:   { dir: 'infernal', w: 23, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 0, cx: 9 }, { f: 's-run-1', top: 0, cx: 9 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 0, cx: 9 }] } } },
    druid:      { dir: 'druid', w: 23, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 10 }, { f: 's-run-1', top: 1, cx: 10 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 1, cx: 9 }] } } },
    tinker:     { dir: 'tinker', w: 20, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 0, cx: 10 }, { f: 's-run-1', top: 0, cx: 9 }, { f: 's-run-2', top: 0, cx: 10 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 0, cx: 10 }] } } },
    ascetic:    { dir: 'ascetic', w: 19, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 2, cx: 9 }, { f: 's-run-1', top: 1, cx: 9 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 1, cx: 10 }] } } },
    starlit:    { dir: 'starlit', w: 24, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 9 }, { f: 's-run-1', top: 1, cx: 10 }, { f: 's-run-2', top: 0, cx: 8 }, { f: 's-run-3', top: 0, cx: 10 }, { f: 's-run-4', top: 1, cx: 10 }] } } },
    monk:       { dir: 'monk', w: 23, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 0, cx: 11 }, { f: 's-run-1', top: 0, cx: 11 }, { f: 's-run-2', top: 0, cx: 11 }, { f: 's-run-3', top: 0, cx: 11 }, { f: 's-run-4', top: 0, cx: 11 }] } } },
    bulwark:    { dir: 'bulwark', w: 20, h: 27, facings: { s: { run: [{ f: 's-run-0', top: 1, cx: 9 }, { f: 's-run-1', top: 1, cx: 9 }, { f: 's-run-2', top: 0, cx: 9 }, { f: 's-run-3', top: 0, cx: 9 }, { f: 's-run-4', top: 1, cx: 9 }] } } },
    colossus:   { dir: 'colossus', w: 26, h: 26, facings: { s: { run: [{ f: 's-run-0', top: 0, cx: 12 }, { f: 's-run-1', top: 0, cx: 12 }, { f: 's-run-2', top: 0, cx: 12 }, { f: 's-run-3', top: 0, cx: 12 }, { f: 's-run-4', top: 0, cx: 12 }] } } },
  ...
  function create(host, opts = {}) {
    ...
    const o = { ink: 'var(--p1)', gear: {}, extras: {}, pal: {}, ghost: false, tint: '', facing: 's', sprite: 'runner', ...opts };
    ...
    html += '<span class="av-extralayer"></span>';
    el.innerHTML = html;
    host.innerHTML = '';
    host.appendChild(el);
    ...
    const extraLayer = el.querySelector('.av-extralayer');
    ...
    const api = {
      ...
      paint() {
        ...
        // Extras are placed in fractions of the box, so they land correctly on
        // every character regardless of that character's size.
        extraLayer.querySelectorAll('.av-extra').forEach((n) => {
          const spec = EXTRAS[n.dataset.slot];
          if (!spec) return;
          const [fx, fy] = spec.at(fr, W, H);
          n.style.width = `${spec.w * 100}%`;
          n.style.left = `${fx * 100}%`;
          n.style.top = `${fy * 100}%`;
          n.style.zIndex = spec.behind ? -2 : 1;
        });
      ...
      setExtras(extras) {
        o.extras = extras || {};
        extraLayer.innerHTML = EXTRA_ORDER.filter((k) => o.extras[k]).map((k) => {
          const spec = EXTRAS[k];
          return `<img class="av-extra av-extra-${k}${spec.spin ? ' av-spin' : ''}"`
            + ` data-slot="${k}" src="${ROOT}${spec.dir}/${o.extras[k]}.png" alt="" draggable="false">`;
        }).join('');
        this.paint();
      },
