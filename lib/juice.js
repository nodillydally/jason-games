...
window.Juice = (function () {
  ...
  function paintSoundBtn() {
    ...
    soundBtn.style.opacity = on ? '1' : '.35';
    soundBtn.style.color = on ? 'var(--ink)' : 'var(--ink-3)';
  }

  function toggleSound() {
    try { localStorage.setItem(SOUND_KEY, soundOn() ? 'off' : 'on'); } catch { /* private mode */ }
    paintSoundBtn();
    if (soundOn()) tone({ freq: 660, dur: 0.09, type: 'triangle', gain: 0.04 });
  }
  ...
  function streakHit(n, el) {
    ...
    if (n % 5 === 0) {
      ...
      buzz([14, 22, 14]);
      chord([660, 880], { dur: 0.14, type: 'triangle', gain: 0.04, stagger: 0.05 });
    }
  }
  ...
  function levelUp(level) {
    ...
    buzz([18, 34, 18, 34]);
    chord([523, 659, 784], { dur: 0.24, type: 'triangle', gain: 0.045, stagger: 0.08 });
  }
  ...
  function celebrate(anchor) {
    ...
    buzz([16, 32, 16]);
    chord([523, 659, 784, 1046], { dur: 0.26, type: 'triangle', gain: 0.04, stagger: 0.075 });
  }
