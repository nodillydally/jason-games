 ...
 *
 *   every answer   → a mono "+120" that lifts and fades, a soft sample, a haptic
...
window.Juice = (function () {
  ...
  function mount() {
    ...
    (window.Theme && window.Theme.dock ? window.Theme.dock() : document.body).appendChild(soundBtn);
    paintSoundBtn();
  ...
  /* ------------------------------ sound ------------------------------ */
  /* Every sound is a recording. There used to be a WebAudio oscillator here,
     pitched off the streak so a run audibly ascended — a nice idea that in
     practice was a bare triangle beep on every correct answer and a sine
     sliding down on every wrong one, which is exactly as cheap as it sounds.
     A sample that fails to load now plays nothing, because silence beats a
     beep. Off by default: a game that starts making noise in a quiet room is
     a game that gets closed. */
  ...
  function toggleSound() {
    ...
    paintSoundBtn();
    if (soundOn()) {
      preload();   // warm the samples now, so the first one isn't the one that misses
      // Confirm audibly once they're in — proof the switch did something.
      setTimeout(() => play('correct', 0.4), 260);
    }
  }
  ...
  /* ------------------------------ samples ------------------------------ */
  /* Five CC0 recordings from Kenney's Interface Sounds pack, chosen by
     measuring spectral centroid rather than by ear — the two that fire on
     every answer sit at the very bottom of the pack's brightness range.
     See lib/sfx/CREDITS.txt. */
  ...
  function loadSample(name) {
    ...
    fetch(SFX_BASE + SAMPLES[name])
      ...
      .catch(() => { buffers.set(name, null); })   // remember it; play() then stays silent
  ...
  function play(name, gain = 0.5) {
    ...
    if (!buf) { loadSample(name); return false; }   // first call warms it, then it's ready
  ...
  return {
    ...
    buzz, play, preload, good, bad, streak: streakHit, levelUp, celebrate,
    reduced,
