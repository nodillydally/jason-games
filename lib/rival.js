...
window.Rival = (function () {
  ...
  // Games name their middle difficulty differently; both are the baseline.
  const DIFF_FACTOR = { easy: 0.82, normal: 1, medium: 1, hard: 1.32 };

  // Each game supplies its own roster at boot: same engine, different
  // characters, because a rival is only characterful when it is fast and slow
  // at the same things the subject is.
  let RIVALS = [];
  const setRoster = (list) => { RIVALS = Array.isArray(list) ? list : []; };

  const clamp = (v, lo, hi) => Math.min(hi, Math.max(lo, v));
  const byId = (id) => RIVALS.find((r) => r.id === id) || RIVALS[0] || null;
